from torch.nn import Conv2d, MaxPool2d, Linear, AvgPool2d, ReLU, Sequential, Module, Dropout, BatchNorm2d
...
class ResidualBlock(Module):
        ...
        def forward(self, x):
            # Realiza uma propagação com os residuos do blocoo anterior
            residual = x
            out = self.conv1(x)
            out = self.conv2(out)
            if self.downsample:
